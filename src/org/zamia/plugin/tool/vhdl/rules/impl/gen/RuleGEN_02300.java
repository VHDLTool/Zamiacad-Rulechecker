		...
		if (reportFile.initialize()) {
			for (ClockSource clockSource : clockSources) {
				for (ViolationPreservationName violation : clockSource.getViolationPreservationName()) {
					...
					reportFile.addSonarTags(info, SonarQubeRule.SONAR_ERROR_GEN_02300, new Object[] {violation.getSignalNameBefore(), violation.getSignalNameAfter()}, SonarQubeRule.SONAR_MSG_GEN_02300, new Object[] {violation.getSignalNameAfter(), violation.getSignalNameBefore()});
				}
			}
